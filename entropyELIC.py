...
class EntropyParameters(nn.Module):
    def __init__(self, in_dim, out_dim, act=nn.GELU) -> None:
        super().__init__()
        self.fusion = nn.Sequential(
            nn.Conv2d(in_dim, in_dim * 3 // 4 + out_dim * 1 // 4, 1),
            act(),
            nn.Conv2d(in_dim * 3 // 4 + out_dim * 1 // 4, in_dim // 2 + out_dim // 2, 1),
            act(),
            nn.Conv2d(in_dim // 2 + out_dim // 2, in_dim // 4 + out_dim * 3 // 4, 1),
            act(),
            nn.Conv2d(in_dim // 4 + out_dim * 3 // 4, out_dim, 1),
        )
