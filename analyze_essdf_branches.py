...
2. Controlled perturbation response:
   - Gaussian blur applied identically and independently to every input band;
   - mean-preserving band-wise scaling that changes relative spectral amplitudes.
...
class ESSDFPreGateExtractor:
    """Extract ESSDF spectral/spatial branch outputs before gating and fusion."""
    ...
    def __call__(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # This follows SSDFN.forward and AnalysisTransform.forward exactly up to
        # the encoder-side ESSDF input.
...
def run_analysis(args: argparse.Namespace) -> None:
    ...
    for batch_index, batch in enumerate(dataloader):
        ...
        for local_index in range(tensor.shape[0]):
            ...
            # Spatial probe: blur the original unpadded image, then apply the same
            # zero padding as the ordinary test pipeline.
            blurred = apply_gaussian_blur(
                x_original, args.blur_kernel_size, args.blur_sigma
            )
            blurred = maybe_clamp(
                blurred,
            ...
            blurred, _ = pad_to_multiple(blurred, args.pad_multiple)
            spectral_blur, spatial_blur = extractor(blurred)
            blur_spectral_response = float(
                relative_feature_response(spectral, spectral_blur, args.eps)[0]
            )
            blur_spatial_response = float(
                relative_feature_response(spatial, spatial_blur, args.eps)[0]
            )
            perturbation_records.append(
                {
                    ...
                    "spectral_response": blur_spectral_response,
                    "spatial_response": blur_spatial_response,
                    "expected_specialization_gap": (
                        blur_spatial_response - blur_spectral_response
                    ),
                    "repeat_count": 1,
    ...
    metadata = {
        ...
        "feature_location": "g_a.decouple spectral_path/spatial_path outputs before gating/fusion",
        ...
        "gaussian_blur": {
            "kernel_size": args.blur_kernel_size,
            "sigma": args.blur_sigma,
            "identical_kernel_for_all_bands": True,
        },
        "bandwise_scaling": {
            "delta": args.band_scale_delta,
            "repeats": args.perturb_repeats,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Analyze pre-gating ESSDF branch similarity and responses to Gaussian blur "
            "and mean-preserving band-wise scaling."
        )
    )
